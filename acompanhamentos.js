(() => {
  ...
  const typeLabel = { vaga: 'Vagas', edital: 'Edital', chamada: 'Lista de espera / chamada', calendario: 'Calendário' };
  let client;
  let user;
  let offers = [];
  let subscriptions = [];
  ...
  const addStyles = () => {
    ...
    style.append(`.waitlist-alert{display:grid;grid-template-columns:auto 1fr;gap:12px;margin:16px 0;padding:14px;border:1px solid #d6e5de;border-radius:12px;background:linear-gradient(135deg,#f4fbf7,#fff)}.waitlist-alert-icon{display:grid;place-items:center;width:34px;height:34px;border-radius:50%;color:#fff;background:#247057;font-size:16px}.waitlist-alert h3{margin:0 0 3px;color:#174c3a;font-family:"Playfair Display",Georgia,serif;font-size:17px}.waitlist-alert p{margin:0;color:#517065;font-size:11px;line-height:1.55}.waitlist-alert small{display:block;margin-top:7px;color:#1e614b;font-size:10px;font-weight:800}`);
  ...
  const nextMonitoringWindow = () => {
    ...
    return `Próxima consulta automática em ${hours}h ${String(minutes).padStart(2, '0')}min.`;
  };
  const refreshMonitoringClock = () => {
    const clock = document.getElementById('waitlistMonitoringClock');
    if (clock) clock.textContent = nextMonitoringWindow();
  };
  ...
  const renderContent = () => {
    ...
    root.innerHTML = `<div class="update-tracker-head"><div><span class="eyebrow" style="color:#a77a34">Acompanhamentos</span><h2>Acompanhe apenas o que importa.</h2><p>Escolha um curso e uma instituição. Quando houver uma atualização confirmada em fonte oficial, ela ficará reunida aqui.</p></div><span class="update-badge">Dados protegidos</span></div>
      ...
      <aside class="waitlist-alert"><span class="waitlist-alert-icon" aria-hidden="true">⌁</span><div><h3>Alerta de lista de espera</h3><p>Ative “Chamadas e listas de espera” ao salvar uma escolha. As fontes oficiais cadastradas são verificadas diariamente; uma novidade só é exibida após a confirmação do documento oficial.</p><small id="waitlistMonitoringClock">${nextMonitoringWindow()}</small></div></aside>
  ...
  const initialize = async () => {
    ...
    addStyles();
    renderTracker();
    window.setInterval(refreshMonitoringClock, 60000);
  };
